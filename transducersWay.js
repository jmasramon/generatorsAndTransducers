...
const filter = (fn) => (reducer) => (acc, val) =>
  fn(val) ? reducer(acc, val) : acc;

// function composition as a reduction
const compose = (...fns) =>
  fns.reduce(
    (acc, val) => (...args) => val(acc(...args)),
    (x) => x
  );
...
const asStream = function* (iterable) {
  yield* iterable;
};
...
const sortedTransformation = (xfMaker, keyFn) => {
...
/**
 * acc::Map
 * val::String "orig->dest"
 */
const countsOf = (acc, val) => {
  if (acc.has(val)) {
...
console.log(
  greatestValue(transduce(transitionKeys, countsOf, new Map(), streamOfLines))
);
